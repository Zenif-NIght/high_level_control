...
TopologyAgent::TopologyAgent(const TopologyAgent& old)
  : graph(old.graph)
  , dist_to_change(old.dist_to_change)
  , goal_index(old.goal_index)
  , goal_calculated(old.goal_calculated)
  , current_goal(old.current_goal)
  , goal(old.goal)
  , odom_topic(old.odom_topic)
  , goal_topic(old.goal_topic)
{
  // Initialize ROS Variables
  pub_goal = n.advertise<geometry_msgs::PoseStamped>(goal_topic, 1);
  sub_odom = n.subscribe(odom_topic, 1, &TopologyAgent::odomCallback, this);
...
void TopologyAgent::generateNewGoal(bool force)
{
  ...
  if (!getNewGoal)
  {  // Check distance to see if close enough to goal
  ...
  if (getNewGoal)
  {
    ...
    // ROS_INFO_STREAM(odom_topic << "dist: " << dist << " Previous: " << goal_index_prev << " Next: " <<
    // static_cast<int>(goal_index));
  }
}
